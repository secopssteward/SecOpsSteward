...
namespace SecOpsSteward.Plugins.Azure.KeyVault
{
    public class AzureKeyVaultServiceConfiguration : AzureSharedConfiguration, IConfigurableObjectConfiguration
    {
        ...
        internal Task<IVault> GetVaultAsync(IAzure azure) => azure.Vaults.GetByResourceGroupAsync(ResourceGroup, KeyVault);
    }
    ...
    public class AzureKeyVaultService : SOSManagedService<AzureKeyVaultServiceConfiguration>
    {
        ...
        private WorkflowTemplateDefinition TEMPLATE_RESET_KEY =>
            new WorkflowTemplateDefinition<AzureKeyVaultService, KeyVaultRegenerateKeyConfiguration>("Reset Key Vault Key")
                .RunWorkflowStep<KeyVaultRegenerateKey>(
                    nameof(KeyVaultRegenerateKeyConfiguration.KeyName).MapsTo(nameof(KeyVaultRegenerateKeyConfiguration.KeyName)));


        private WorkflowTemplateDefinition TEMPLATE_RESET_SECRET =>
            new WorkflowTemplateDefinition<AzureKeyVaultService, KeyVaultRegenerateSecretConfiguration>("Reset Key Vault Secret")
                .RunWorkflowStep<KeyVaultRegenerateSecret>(
                    nameof(KeyVaultRegenerateSecretConfiguration.SecretName).MapsTo(nameof(KeyVaultRegenerateSecretConfiguration.SecretName)),
                    nameof(KeyVaultRegenerateSecretConfiguration.Length).MapsTo(nameof(KeyVaultRegenerateSecretConfiguration.Length)))
