    ...
    public class SecurityTripwire
    {
        ...
        public void HandleTripwire(SecurityTripwireConditions condition, ActiveWorkflow workflow)
        {
            if (_handlers.Count() > 1)
                ...
                _logger.LogWarning("There are no registered tripwire handlers, but one was activated for condition {condition}", condition);

            foreach (var handler in _handlers)
            {
                try { handler(condition, workflow); }
                catch (Exception ex) { _logger.LogError(ex, "Tripwire handler crashed while processing an incoming tripwire!"); }
            }
        }
    }
}
