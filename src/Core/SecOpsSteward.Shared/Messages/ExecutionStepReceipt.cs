...
namespace SecOpsSteward.Shared.Messages
{
    ...
    public class ExecutionStepReceipt : ISignableByOne, IEncryptable
    {
        ...
        /// </summary>
        [NotSigned]
        public PluginOutputStructure PluginResult { get; set; } = new PluginOutputStructure(string.Empty);
        ...
        {
            return $"Step {StepId} - Ran from {ExecutionStarted} to {ExecutionEnded} ({(ExecutionEnded - ExecutionStarted).TotalSeconds}s) - Result: {StepExecutionResult} - {Signature}";
        }
    }
}
