...
namespace SecOpsSteward.Shared.Cryptography
{
    ...
    public static class ISignableObjectExtensions
    {
        ...
        {
            var dataHash = GetSignableHash(signable);
        ...
        /// <param name="signatureService">Signature service to process verify request</param>
        /// <returns><c>TRUE</c> if the object has been unmodified since its signature, otherwise <c>FALSE</c></returns>
        public static async Task<Dictionary<ChimeraEntitySignature, bool>> Verify(this ISignableByMany signable, ICryptographicService signatureService)
        {
            var results = new Dictionary<ChimeraEntitySignature, bool>();

            var baseHash = GetSignableHash(signable); // data base hash (without signatures)
        ...
        /// <returns></returns>
        public static async Task Sign(this ISignableByOne signable, ICryptographicService signatureService, ChimeraEntityIdentifier signingEntity, string signatureDisplay)
        {
        ...
        /// <param name="signatureDisplay">Display for signer's information</param>
        /// <returns></returns>
        public static async Task Sign(this ISignableByMany signable, ICryptographicService signatureService, ChimeraEntityIdentifier signingEntity, string signatureDisplay)
        {
            var newSignature = new ChimeraEntitySignature()
            {
                Timestamp = DateTimeOffset.Now,
            ...
            allHashes.Add(GetSignableHash(signable)); // data
