...
namespace SecOpsSteward.Plugins
{
    public static class TemplatedStrings
    {
        ...
        private static Regex PathExtractor = new Regex("{{\\$(.*?)}}");
        private static Regex BooleanCheck = new Regex("{{\\$(.*?)\\?(.*?)\\:(.*?)}}");
                ...
                if (BooleanCheck.IsMatch(replace))
                    foreach (Match bMatch in BooleanCheck.Matches(replace))
                    {
                        ...
                        if (!bool.TryParse(values.SharedOutputs[bMatch.Groups[1].Value], out val))
                            throw new System.Exception("Value was not a boolean but template expected a boolean");
                        if (val)
                            value = optA;
                        else value = optB;
                    }
                else if (values.SharedOutputs.Contains(match.Groups[1].Value))
                    value = (string)values.SharedOutputs[match.Groups[1].Value];
                else throw new Exception("Shared Outputs does not contain expected key '" + match.Groups[1].Value + "'");
        ...
        public static List<string> GetRequiredInputsForTemplateString(string templateString) =>
            PathExtractor.Matches(templateString).Select(v => v.Groups[1].Value).Distinct().ToList();
    }
}
