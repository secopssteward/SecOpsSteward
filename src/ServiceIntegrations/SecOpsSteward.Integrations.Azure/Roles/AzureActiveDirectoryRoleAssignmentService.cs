...
namespace SecOpsSteward.Integrations.Azure.Roles
{
    public class AzureActiveDirectoryRoleAssignmentService : AzurePlatformIntegrationBase, IRoleAssignmentService
    {
        ...
        private string ROLE_NAME_PREFIX = "SoSRole - ";
        private string ROLE_DESCRIPTION_PREFIX = "Role to ";

        public async Task<bool> ApplyScopedRoleToIdentity(ChimeraEntityIdentifier Entity, AssignableRole Role, string Scope) => await CreateRoleAssignment(GetOid(Entity), Role, Scope);
        public async Task<bool> RemoveScopedRoleFromIdentity(ChimeraEntityIdentifier Entity, AssignableRole Role, string Scope) => await DropRoleAssignment(GetOid(Entity), Role, Scope);

        public async Task<TokenOwner> ResolveUsername(string username)
        {
            var user = await _platformFactory.GetCredential().GetAzure().AccessManagement.ActiveDirectoryUsers.GetByNameAsync(username);
            return BuildRecord(user, false) as TokenOwner;
        }
        ...
        private async Task<bool> CreateRoleAssignment(string oid, AssignableRole role, string scope)
        {
            ...
            try
            {
                Logger.LogTrace("Adding role {role} to OID {oid} on scope {scope}", role, Guid.Parse(oid).ShortId(), scope);
            ...
            catch (Exception ex)
            {
                if (ex.Message.Contains("already exists"))
                    return true;
                Logger.LogWarning("FAILED adding role {role} to OID {oid} on scope {scope}", role, Guid.Parse(oid).ShortId(), scope);
                throw;
            }
        }
        ...
        protected async Task<bool> DropRoleAssignment(string oid, AssignableRole role, string scope)
        {
            try
            {
                ...
                var assignments = _platformFactory.GetCredential().GetAzure().AccessManagement.RoleAssignments.ListByScope(scope);
                var target = assignments.FirstOrDefault(s => s.PrincipalId == oid && s.RoleDefinitionId == roleId);
                if (target != null)
                {
                    await _platformFactory.GetCredential().GetAzure().AccessManagement.RoleAssignments.DeleteByIdAsync(target.Id);
                    return true;
                }
            }
            catch (Exception)
        ...
        private async Task<string> GetDefinitionId(string scope, AssignableRole role) =>
            (await _platformFactory.GetCredential().GetAzure().AccessManagement.RoleDefinitions.Inner
        ...
        private string GetRoleId(AssignableRole role)
        {
            switch (role)
            {
                ...
                case AssignableRole.CanManagePermissionsOnVault: return "18d7d88d-d35e-4fb5-a5c3-7773c20a72d9"; // user access admin
        ...
        public async Task<bool> HasAssignedPluginRole<TPlugin>(TPlugin plugin, IEnumerable<PluginRbacRequirements> requirements, string identity)
        ...
        public async Task AssignPluginRole<TPlugin>(TPlugin plugin, IEnumerable<PluginRbacRequirements> requirements, string identity)
            where TPlugin : IPlugin
        {
            var azure = _platformFactory.GetCredential().GetAzure();
        ...
        public async Task UnassignPluginRole<TPlugin>(TPlugin plugin, IEnumerable<PluginRbacRequirements> requirements, string identity)
            where TPlugin : IPlugin
        {
            var azure = _platformFactory.GetCredential().GetAzure();
        ...
        private Guid PluginRoleDefinitionGuid(IPlugin plugin) => CreateCombinedGuid(plugin.GenerateId(), Guid.Parse(CfgSubscriptionId));
        private string PluginRoleDefinitionId(IPlugin plugin) => $"/subscriptions/{CfgSubscriptionId}/providers/Microsoft.Authorization/roleDefinitions/{PluginRoleDefinitionGuid(plugin)}";
            ...
            string[] dataActions)
        {
            ...
            try
            {
                roleDefinition = await azure.AccessManagement.RoleDefinitions.GetByIdAsync(PluginRoleDefinitionId(plugin));
            }
            catch { }
            ...
            if (roleDefinition == null)
            {
                ...
                .Inner.CreateOrUpdateWithHttpMessagesAsync(
                    ...
                    new RoleDefinitionInner(
                        id: PluginRoleDefinitionId(plugin),
                        roleType: "CustomRole",
        ...
        {
            _ = await azure.AccessManagement.RoleDefinitions
                .Inner.DeleteWithHttpMessagesAsync($"/subscriptions/{CfgSubscriptionId}", PluginRoleDefinitionId(plugin));
        }
